...
def build_augmentation_pipeline() -> keras.Sequential:
    ...
    # ============================================================
    # TODO (Aufgabe 1): Baue die Augmentation-Schichten auf.
    #   Prüfe jeweils den passenden CONFIG-Schalter und hänge – falls
    #   aktiv – die entsprechende Keras-Preprocessing-Schicht an
    #   `aug_layers` an:
    #     - CONFIG["AUG_FLIP_HORIZONTAL"]  -> layers.RandomFlip("horizontal")
    #     - CONFIG["AUG_ROTATION"]  > 0    -> layers.RandomRotation(...)
    #     - CONFIG["AUG_ZOOM"]      > 0    -> layers.RandomZoom(...)
    #     - CONFIG["AUG_CONTRAST"]  > 0    -> layers.RandomContrast(...)
    # Hinweis: Diese Schichten liegen in `tensorflow.keras.layers` und
    #          werden nur beim Training (training=True) aktiv.
    # ============================================================
    raise NotImplementedError("TODO: Augmentation-Pipeline aufbauen")
...
def build_cnn_model(
    ...
    # ============================================================
    # TODO (Aufgabe 2): Baue die Convolutional-Blöcke auf.
    #   Iteriere über CONFIG["CONV_FILTERS"] und erzeuge pro Eintrag
    #   einen Block auf dem Tensor `x`:
    #     Conv2D(filters, CONFIG["CONV_KERNEL"], padding="same",
    #            kernel_regularizer=regularizers.l2(CONFIG["L2"]))
    #     -> optional BatchNormalization (falls CONFIG["BATCH_NORM"])
    #     -> Activation("relu")
    #     -> MaxPooling2D(pool_size=2)
    # Hinweis: Functional API – jede Schicht wird mit (x) aufgerufen
    #          und das Ergebnis wieder `x` zugewiesen.
    # ============================================================
    raise NotImplementedError("TODO: Convolutional-Blöcke implementieren")

    # ── Übergang zu Dense ─────────────────────────────────
    x = layers.Flatten(name="flatten")(x)

    # ============================================================
    # TODO (Aufgabe 3): Baue den Klassifikationskopf.
    #   Iteriere über CONFIG["DENSE_UNITS"] und erzeuge pro Eintrag:
    #     Dense(units, kernel_regularizer=regularizers.l2(CONFIG["L2"]))
    #     -> optional BatchNormalization (falls CONFIG["BATCH_NORM"])
    #     -> Activation("relu")
    #     -> Dropout(CONFIG["DROPOUT"])
    #   Definiere danach die Ausgabeschicht `outputs`:
    #     Dense(num_classes, activation="softmax")
    # Hinweis: Softmax liefert eine Wahrscheinlichkeitsverteilung
    #          über die num_classes Klassen.
    # ============================================================
    raise NotImplementedError("TODO: Dense-Kopf und Ausgabeschicht implementieren")

    model = keras.Model(inputs=inputs, outputs=outputs, name="cnn_classifier")

    # ============================================================
    # TODO (Aufgabe 4): Kompiliere das Modell.
    #   Rufe model.compile(...) auf mit:
    #     - optimizer = keras.optimizers.Adam(learning_rate=CONFIG["LEARNING_RATE"])
    #     - loss      = "sparse_categorical_crossentropy"
    #     - metrics   = ["accuracy"]
    # Hinweis: "sparse_categorical_crossentropy" passt zu ganzzahligen
    #          Labels (0..num_classes-1) statt One-Hot-Vektoren.
    # ============================================================
    raise NotImplementedError("TODO: Modell kompilieren")
