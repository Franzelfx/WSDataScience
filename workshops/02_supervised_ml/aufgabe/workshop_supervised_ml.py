...
Datengenerierung, Plot-Funktionen, argparse, main(), Speichern/Laden) ist bereits
...
markiert. Die vollständige Musterlösung findest du unter:
    ../loesung/workshop_supervised_ml.py
...
def build_preprocessor(cfg: dict) -> ColumnTransformer:
    ...
    """
    # ============================================================
    # TODO (Aufgabe 1): Preprocessing-Pipeline mit ColumnTransformer aufbauen.
    ...
    #   - preprocessor: ColumnTransformer mit zwei Transformern:
    #       ("num", numeric_pipeline,     cfg["NUMERIC_FEATURES"])
    #       ("cat", categorical_pipeline, cfg["CATEGORICAL_FEATURES"])
    # Hinweis: handle_unknown="ignore" macht die Inferenz robust gegen
    #          Kategorien, die im Training nicht vorkamen. Alles gehört IN die
    #          Pipeline, damit fit nur auf Trainingsdaten passiert (kein Leakage).
    # ============================================================
    raise NotImplementedError("TODO (Aufgabe 1): ColumnTransformer/Preprocessing aufbauen")
...
def build_pipeline(name: str, cfg: dict) -> Pipeline:
    ...
    # ============================================================
    # TODO (Aufgabe 2): Eine sklearn-Pipeline aus zwei Schritten bauen:
    #   1) ("preprocessor", build_preprocessor(cfg))
    #   2) ("classifier",   build_classifier(name, cfg))
    # Hinweis: Pipeline(steps=[(name, objekt), ...]). Weil Preprocessing UND
    #          Modell in EINEM Objekt stecken, kann die ganze Pipeline sauber
    #          per Cross-Validation bewertet und mit joblib gespeichert werden.
    # ============================================================
    raise NotImplementedError("TODO (Aufgabe 2): Modell-Pipeline (preprocessor + classifier) bauen")
...
def compare_models(X_train, y_train, cfg: dict) -> dict:
    ...
    # ============================================================
    # TODO (Aufgabe 3): Modellvergleich per Cross-Validation auswerten.
    #   Für jeden Namen in ["logreg", "rf", "gb"]:
    #     - pipe = build_pipeline(name, cfg)
    #     - scores = cross_validate(pipe, X_train, y_train, cv=cv,
    #                               scoring=scoring, n_jobs=-1)
    #     - in results[name] ablegen
    #     - je Metrik den Mittelwert und die Standardabweichung ausgeben, z.B.:
    #         vals = scores[f"test_{metric}"]
    #         print(f"      {metric:18s}: {vals.mean():.3f} ± {vals.std():.3f}")
    #   Danach: best_name = Modell mit dem höchsten Mittelwert der Primär-Metrik
    #           primary = cfg["PRIMARY_SCORING"]; Zugriff: results[k][f"test_{primary}"]
    # Hinweis: cross_validate liefert ein dict mit Schlüsseln "test_<metrik>",
    #          jeder Wert ist ein numpy-Array mit einem Score pro Fold.
    # ============================================================
    raise NotImplementedError("TODO (Aufgabe 3): Cross-Validation auswerten & bestes Modell bestimmen")
    # Die folgenden Variablen musst du oben füllen:
    #   results   -> dict {modellname: cross_validate-Ergebnis}
    #   best_name -> str, bestes Modell laut Primär-Metrik
    #   best_score -> float, dessen mittlerer Score
...
def choose_threshold(y_true, y_proba, cfg: dict) -> dict:
    ...
    # ============================================================
    # TODO (Aufgabe 4): Entscheidungsschwelle nach Business-Kosten wählen.
    #   (a) Kosten-optimal: für jede Schwelle thr in pr_thresholds
    #         y_pred = (y_proba >= thr)
    #         fp = Anzahl (y_pred==1 & y_true==0)   -> Fehlalarm
    #         fn = Anzahl (y_pred==0 & y_true==1)   -> übersehener Churner
    #         total_cost = fn*cost_fn + fp*cost_fp
    #       Sammle alle Kosten in einem Array `costs` und nimm das Minimum:
    #         best_idx = int(np.argmin(costs)); cost_threshold = pr_thresholds[best_idx]
    #   (b) Recall-Ziel: kleinste Schwelle, die recall >= target_recall erfüllt,
    #       darunter die mit der besten Precision -> recall_threshold.
    #       (Falls keine Schwelle den Recall erreicht: recall_threshold = 0.5)
    # Hinweis: precision/recall/pr_thresholds stammen aus precision_recall_curve
    #          (oben schon berechnet und auf gleiche Länge gebracht). Ein hoher
    #          COST_FN drückt die Schwelle nach unten (mehr Recall, weniger
    #          verpasste Churner) – das ist der Kern der Threshold-Wahl.
    # ============================================================
    raise NotImplementedError("TODO (Aufgabe 4): Threshold nach Kosten und Recall-Ziel wählen")
    # Erwartete Ergebnisse (oben zu füllen):
    #   costs            -> np.ndarray der Gesamtkosten je Schwelle
    #   cost_threshold   -> float, kostenminimale Schwelle
    #   recall_threshold -> float, Schwelle für das Recall-Ziel
