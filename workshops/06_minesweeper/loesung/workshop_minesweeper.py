...
def encode(view: np.ndarray) -> np.ndarray:
    ...
    enc[..., 0] = (view == HIDDEN).astype(np.float32)
    for k in range(9):                       # Zahlen 0..8 -> Kanäle 1..9
        enc[..., k + 1] = (view == k).astype(np.float32)
    return enc
...
class DQNAgent:
    ...
    def _build(self):
        ...
        x = inp
        for f in self.cfg["CONV_FILTERS"]:
            x = layers.Conv2D(f, 3, padding="same", activation="relu")(x)
        x = layers.Conv2D(1, 1, padding="same", activation="linear")(x)   # 1 Q-Wert pro Zelle
        out = layers.Flatten()(x)                                          # -> ROWS*COLS
    ...
    def replay(self):
        ...
        # Ungültige Folgeaktionen (bereits aufgedeckte Zellen) maskieren
        next_masks = np.stack([(b[3].reshape(-1) == HIDDEN) for b in batch])
        q_next = np.where(next_masks, q_next, -1e9)
        max_next = np.max(q_next, axis=1)

        targets = self.model(states, training=False).numpy()
        targets[np.arange(self.batch_size), actions] = rewards + self.gamma * max_next * (1.0 - dones)
