...
# aus dem aufgabe/-Unterordner heraus).
...
def build_autoencoder(input_dim: int, cfg: dict) -> keras.Model:
    ...
    # ============================================================
    # TODO (Aufgabe 1): Baue den Autoencoder (Encoder -> Latent -> Decoder).
    #
    # Encoder:
    #   - Beginne mit x = inputs.
    #   - Iteriere über cfg["HIDDEN_DIMS"] und hänge je einen
    #     layers.Dense(units, activation=cfg["ACTIVATION"], kernel_regularizer=l2) an.
    #   - Falls cfg["DROPOUT"] > 0, füge nach jedem Dense-Layer ein
    #     layers.Dropout(cfg["DROPOUT"]) ein.
    #
    # Latent (Engpass / Bottleneck):
    #   - Ein Dense-Layer mit cfg["LATENT_DIM"] Einheiten, activation=None,
    #     name="latent"  (der Name wird später für den Latent-Space-Plot gebraucht!).
    #
    # Decoder (spiegelt den Encoder):
    #   - Iteriere über reversed(cfg["HIDDEN_DIMS"]) mit
    #     layers.Dense(units, activation=cfg["ACTIVATION"]).
    #   - Ausgabelayer: layers.Dense(input_dim, activation=None, name="outputs").
    #
    # Hinweis: Verwende die Keras Functional API: x = layers.Dense(...)(x).
    #          Der Output muss dieselbe Dimension (input_dim) wie der Input haben.
    # ============================================================
    raise NotImplementedError("TODO Aufgabe 1: Encoder-/Decoder-Architektur aufbauen.")
...
def reconstruction_errors(model: keras.Model, X: np.ndarray) -> np.ndarray:
    # ============================================================
    # TODO (Aufgabe 2): Berechne den Rekonstruktionsfehler pro Datenpunkt.
    #
    #   1. Lasse das Modell die Eingaben rekonstruieren:
    #      X_pred = model.predict(X, verbose=0)
    #   2. Berechne den mittleren quadratischen Fehler (MSE) je Zeile:
    #      errors = np.mean(np.square(X - X_pred), axis=1)
    #
    # Hinweis: axis=1 mittelt über die Features, sodass ein Fehlerwert
    #          pro Datenpunkt übrig bleibt (Basis der Anomalie-Erkennung).
    # ============================================================
    raise NotImplementedError("TODO Aufgabe 2: Rekonstruktionsfehler pro Datenpunkt berechnen.")
...
def main():
    ...
    # ============================================================
    # TODO (Aufgabe 3): Trainiere den Autoencoder.
    #
    # Rufe model.fit(...) auf und speichere das Ergebnis in `history`.
    # Wichtig: Ein Autoencoder rekonstruiert seine Eingabe, d.h. Input UND
    # Ziel sind (fast) dieselben Daten:
    #   - Input:  X_train_in  (ggf. verrauscht, für Denoising-AE)
    #   - Ziel:   X_train     (immer die sauberen Originaldaten!)
    #
    # Übergebe außerdem:
    ...
    # ============================================================
    raise NotImplementedError("TODO Aufgabe 3: Autoencoder trainieren (model.fit).")
